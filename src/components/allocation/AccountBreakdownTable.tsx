...
export function AccountBreakdownTable({
  ...
  if (!collapsible) {
    return <BreakdownContent accounts={accounts} />;
  }
  ...
  return (
    <details className="group mt-1">
      ...
      <BreakdownContent accounts={accounts} />
    </details>
  );
}

function BreakdownContent({ accounts }: { accounts: AccountBreakdown[] }) {
  ...
  return (
    <div className="mt-3 flex flex-col divide-y divide-avocado-100">
      ...
      {accounts.map((acct) => {
        ...
        const isEmpty = Number(acct.total) === 0;
        ...
        return (
          <div key={acct.accountId} className="flex items-center gap-6 py-2">
            <div className="flex min-w-0 flex-1 flex-col gap-1">
              ...
              {isEmpty ? (
                <span className="text-xs text-avocado-600">No holdings yet</span>
              ) : (
            ...
            <div className={`${tableWidth} shrink-0`}>
              <div className={`grid ${colClass} items-center text-center text-xs`}>
                {visibleSlices.map((b) => (
                  ...
                  >
                    {isEmpty || b.current <= 0.001 ? "—" : formatPercent(b.current)}
                  </span>
                ))}
              </div>
            </div>
